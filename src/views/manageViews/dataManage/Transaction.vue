...
<template>
  <div>
    ...
    >
      ...
      >
        <el-form-item label="用户昵称" prop="userNickname">
          <el-input v-model="infoForm.userNickname" clearable></el-input>
        </el-form-item>
        ...
        <el-form-item label="交易状态" prop="transactionStatus">
          <el-input v-model="infoForm.transactionStatus" clearable></el-input>
        </el-form-item>

        <el-form-item label="交易金额" prop="transactionAmount">
          <el-input v-model="infoForm.transactionAmount" clearable></el-input>
        </el-form-item>
        ...
        <el-form-item label="日期时间">
          <el-form-item prop="transactionTime">
            <el-date-picker
                type="datetime"
                placeholder="选择日期时间"
                v-model="infoForm.transactionTime"
                style="width: 100%"
            ></el-date-picker>
          </el-form-item>
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="userNickname" sortable label="用户昵称">
      </el-table-column>
      <!--      <el-table-column prop="touristId" sortable label="用户编号">-->
      <!--      </el-table-column>-->
      <!--      -->
      <el-table-column prop="storeName" sortable label="商店名称">
      </el-table-column>
      <!--      <el-table-column prop="storeId" sortable label="商店编号">-->
      <!--      </el-table-column>-->
      <!--      -->
      <el-table-column prop="transactionAmount" sortable label="交易金额">
        <template slot-scope="scope">
          <el-tag size="medium" :type="randomTag()" v-if="scope.row.transactionAmount">{{
              "￥ " + scope.row.transactionAmount + " 元"
            }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="transactionStatus" sortable label="交易状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.transactionStatus === 1" effect="dark">正常</el-tag>
...
export default {
  ...
  methods: {
    ...
    //删除一个用户
    delete(id) {
      this.request.delete("/transaction/" + id).then((res) => {
        if (res) {
    ...
    submitForm(infoForm) {
      this.$refs[infoForm].validate((valid) => {
        if (valid) {
          this.request.post("/transaction", this.infoForm).then((res) => {
            if (res) {
              // console.log(res);
