  ...
  <div>
    ...
    >
      <el-form
          ...
          class="demo-ruleForm"
      >
        <el-form-item label="评价人昵称" prop="commenterNickname">
          <el-input v-model="infoForm.commenterNickname" clearable></el-input>
        </el-form-item>
        <el-form-item label="评价关键词" prop="commentKeyword">
          <el-input v-model="infoForm.commentKeyword" clearable></el-input>
        </el-form-item>
    ...
    >
      ...
      <el-table-column prop="commenterNickname" sortable label="评价人昵称">
      </el-table-column>

      <el-table-column prop="commentKeyword" sortable label="评论关键词">
        <template slot-scope="scope">
          <el-tag
              effect="dark"
              size="medium"
              type="success"
              v-if="scope.row.commentKeyword === '满意'"
          >{{ scope.row.commentKeyword }}
          </el-tag
          >
          <el-tag
              effect="dark"
              size="medium"
              v-else-if="scope.row.commentKeyword === '比较满意'"
          >{{ scope.row.commentKeyword }}
          </el-tag
          >
          <el-tag
              ...
              size="medium"
              type="info"
              v-else-if="scope.row.commentKeyword === '一般'"
          >{{ scope.row.commentKeyword }}
          </el-tag
          >
          <el-tag
              ...
              size="medium"
              type="warning"
              v-else-if="scope.row.commentKeyword === '不太满意'"
          >{{ scope.row.commentKeyword }}
          </el-tag
          >
          <el-tag
              ...
              size="medium"
              type="danger"
              v-else-if="scope.row.commentKeyword === '不满意'"
          >{{ scope.row.commentKeyword }}
          </el-tag
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    //删除一个用户
    delete(id) {
      this.request.delete("/userComment/" + id).then((res) => {
        if (res) {
    ...
    submitForm(infoForm) {
      this.$refs[infoForm].validate((valid) => {
        if (valid) {
          this.request.post("/userComment", this.infoForm).then((res) => {
            if (res) {
              // console.log(res);
